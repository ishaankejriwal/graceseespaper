...
from gracefc.experiment_lstm import run_lstm_experiment  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--tag", default="phase7_lstm")
    ap.add_argument("--smoke", action="store_true",
                    help="fold f1, horizon 1, 2 placebo seeds, 1 LSTM seed")
    args = ap.parse_args()
    ...
    if args.smoke:
        ...
        kwargs = {"folds": DEFAULT_FOLDS[:1], "horizons": range(1, 2), "lstm_seeds": (0,)}
    print(f"sample: {wide.shape[1]} basins | placebo seeds={args.placebo}")

    pred_rows, plac_monthly = run_lstm_experiment(
